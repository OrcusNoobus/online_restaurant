...
- `src/server/db/schema.ts` — `staff_role` enum, `staff_users`,
  `staff_sessions`, `order_status_events`, `restaurant_settings` (single
  row + `catalog_owned_by_admin_since`), `products.ingredients/allergens`,
...
- `src/proxy.ts` — optimistic redirect to `/admin/login` when the session
  cookie is missing on `/admin/:path*` (new)
...
- `src/app/admin/layout.tsx` — shell: nav, current user, logout (new)
- `src/app/admin/login/page.tsx` (new)
- `src/app/admin/page.tsx` — orders day view: 5s polling, new-order
...
- `src/app/admin/produse/page.tsx` — catalog: availability toggles (staff),
  price edits, texts + ingredients/allergens, new product/category (admin) (new)
- `src/app/admin/zone/page.tsx`, `src/app/admin/setari/page.tsx` — admin-only (new)
...
- **Order lifecycle (D4/D5):** graph lives in `src/lib/order-status.ts`;
  ...
  UPDATE orders.status (+ estimateMinutes when accepting) + INSERT
  order_status_events. Undo reverts to the latest event's `from_status` by
  inserting a compensating event — the journal never loses history. Snapshots
  are never touched.
- **Settings (D6/D7):** single typed row read per request by checkout
  services and the admin; `schedule.ts` keeps pure signatures
  `(config, now) → verdict`. First admin write to catalog/zones/settings sets
  `catalog_owned_by_admin_since`; `scripts/seed.ts` exits loudly when set.
...
- **Catalog admin:** mutations in `catalog-admin.ts` repository; slugs
  ...
  staff-permitted endpoints/fields per the matrix. Public menu/pricing
  already filter `active` — variants gain the same flag and filters.
...
- **Seed guard vs test suite:** integration tests that flip catalog ownership
  reset it in cleanup; the suite's own seeding continues to work on the dev
  DB. Asserted by an explicit test (seed refuses → SEED_FORCE succeeds).
